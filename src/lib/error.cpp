...
stdext_NODISCARD const char* stdext::enum_to_string( stdext::InternalError err ) {
  switch ( err ) {
    case stdext::InternalError::AssertionFailed:
      return "InternalError::AssertionFailed";
    default:
      return nullptr;
  }
}
...
void stdext::PRINTERROR( const char* const file, const int line, const char* const func, const char* const message ) {
  ...
  if (file) {
    ...
    file_copy = new char[sz+1]; file_copy[sz] = 0;
...
void stdext::EPICFAIL_RET() {
  std::fprintf( stdout, abort_msg );
}

void stdext::EPICFAIL_RET_VOID() {
  std::fprintf( stdout, abort_msg );
}

void stdext::EPICFAIL_RET_INT() {
  std::fprintf( stdout, abort_msg );
}

void stdext::EPICFAIL_LOOP() {
  std::fprintf( stdout, skipping_iteration_msg );
}

void stdext::EPICFAIL_LOOP_BREAK() {
  std::printf( abort_loop_msg );
}

void stdext::EPICFAIL_NOP() {
  std::printf( continue_msg );
}
